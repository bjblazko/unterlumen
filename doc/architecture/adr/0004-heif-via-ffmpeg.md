...
Convert HEIF/HEIC files to JPEG on-the-fly by invoking ffmpeg as a subprocess. The converted output is piped to stdout. Converted results are cached to disk in `~/Library/Caches/unterlumen/` (via `os.UserCacheDir()`, falling back to `$TMPDIR/unterlumen-cache/`) to avoid re-running ffmpeg on repeated requests for the same file.
...
- **Disk cache** — Converted JPEGs are stored in `~/Library/Caches/unterlumen/` (macOS) via `os.UserCacheDir()`, falling back to `$TMPDIR/unterlumen-cache/`. Cache keys are derived from file path only (modification time was removed so cached conversions are found even when the source volume is offline). Entries survive reboots under the persistent cache path. No explicit cleanup; OS cache-eviction policy handles size management. This does not conflict with ADR-0002 because nothing is written to the photo directory.
