...
### Security
- **Unescaped filenames rendered as HTML in grid/list/justified views (stored XSS)** — `entry.name`/`entry.label` were interpolated directly into `data-name`, `alt`, and item-name markup with no escaping, unlike comparable templates elsewhere that already used the existing `escapeHtml()` helper. A filename containing `">` or an HTML tag (e.g. from an SD card import, shared drive, or zip extraction) could break out of an attribute or inject markup into the page. Same fix applied to a matching gap in the channels list. All affected templates now escape filenames/paths before interpolation.
- **Path traversal in `pathguard.SafePath` via sibling-directory prefix collision** — the boundary check used `strings.HasPrefix(resolved, resolvedRoot)` with no trailing separator, so a directory whose name merely started with the root's name (e.g. a `photos-old` or `photosEVIL` next to `photos`) passed the check. A request could read/list/copy/move/delete outside the browse root by resolving into such a sibling. Affects `SafePath`, used by nearly every filesystem-touching endpoint (browse, fileops, crop, location, library, export). Fixed by requiring an exact match or a `root + separator` prefix, matching the already-correct check in `SafePathLogical` — with the separator appended only when root doesn't already end in one, so root `/` (no navigation restriction — a supported, real configuration) doesn't turn the prefix into `//`, which matches nothing and would reject every path.

### Fixed
- **Cleanup/reindex scoped to one folder could purge a sibling folder's photos** — folder-scoped SQL `LIKE` queries (`ListPhotoRefsInFolder`, `DeletePathCacheForFolder`, and the path-prefix filters behind Statistics/Timeline) built patterns like `"<folder>/%"` without escaping `%`/`_`, and SQLite's `_` wildcard matches any single character. A folder named e.g. `2024_Trip` also matched an unrelated sibling like `2024XTrip`, so "index new photos"/"cleanup" scoped to one folder could mark the sibling's photos missing and purge them from the library database. Folder-name characters are now escaped before being used in `LIKE` patterns.
- **Deleting or renaming a library-tracked file outside the library UI left a stale, broken entry** — the generic `/api/delete` and `/api/rename` endpoints (used by browse mode, Commander, and the wastebin) never notified the library manager, unlike Copy/Move which already sync on success. A deleted or renamed photo kept showing up in library view — pointing at a file that no longer existed — until a manual rescan. Delete now triggers a background library cleanup for the affected folder; rename synchronizes the library index immediately for files, and triggers a cleanup for directory renames.
- **A `sql.DB` handle could survive `DeleteLibrary` under concurrent access** — `getDB`'s handle-opening path ran unsynchronized with `DeleteLibrary`'s evict-then-remove, so a request could reopen and re-cache a `*sql.DB` for a library whose directory was being deleted, leaving a phantom in-memory handle after the delete completed. Handle creation is now serialized against deletion.
- **Cropping an oriented WebP photo cropped the wrong region** — unlike the JPEG/HEIF crop paths, `cropWebP` computed the crop rectangle against the raw, un-rotated pixel buffer instead of applying EXIF orientation first, so a WebP photo with a 90°/270° orientation tag was cropped in the wrong location.
- **External tool calls (ffmpeg, sips, heif-convert, exiftool, cwebp) had no timeout** — a malformed or pathological input file that made one of these hang could block the handling goroutine indefinitely. All external process calls are now bounded by a timeout.
- **"Open in Commander" could target the wrong pane in cross-library list-search mode** — a second, out-of-date pane-resolution helper in library mode didn't know about the list-view cross-library search pane, unlike the canonical helper used for keyboard/info-panel routing. Removed the duplicate; both now go through the same helper.
- **Scanning a duplicate file at a second path could orphan the original from the library** — content-addressed photo IDs mean indexing a byte-identical copy at a new location unconditionally repointed that photo's single database row at the new path, even though the original file was untouched and still on disk — it just silently stopped appearing in the library. Indexing now keeps pointing at the original path when it still exists on disk, only repointing when the previous location is actually gone (a genuine rename).

### Changed
- **Updated dependencies**: `modernc.org/sqlite` v1.50.1 → v1.57.0 (picks up a fix for a data-corruption bug in multi-database journal rollback; also makes the `_foreign_keys=on` DSN parameter — already set in our connection string — actually take effect, where it was previously silently ignored), `golang.org/x/image` v0.41.0 → v0.45.0, `github.com/yuin/goldmark` v1.8.2 → v1.8.5 (disables SVG in `data:` image URLs and fixes two parser crash bugs — relevant since it renders self-authored about/imprint markdown with raw HTML enabled), plus transitive bumps (`go-isatty`, `modernc.org/libc`, `modernc.org/memory`, `golang.org/x/sys`). `@playwright/test` (e2e only) v1.44.0 → v1.62.1.
...
- **Read-ahead prefetch** — The viewer now prefetches the next two images on navigation, reducing latency when browsing photos from a NAS. HEIF/HEIC images also benefit from a server-side in-memory LRU cache (20 entries) and improved HTTP cache headers (`Cache-Control: private, max-age=3600` with ETag), enabling browser caching for the duration of a session.
- **Generated site — mobile UX improvements** — The published photo-album website is now better optimised for phones and tablets:
  ...
  - **Full-screen lightbox with swipe** — tapping a photo opens a true full-screen viewer (black background, image fills the entire screen, `object-fit: contain` for correct aspect ratio in portrait and landscape). On touch devices the arrow buttons are hidden; swipe left/right navigates between photos. Arrow keys and the ✕ button still work everywhere. `viewport-fit=cover` allows the lightbox to reach the edges of iPhones with a notch or home indicator.
  - **No horizontal scroll** — the long contact URL in the footer could push the layout wider than the viewport on narrow screens; the footer now wraps and contact links truncate with an ellipsis. `overflow-x: hidden` on `html` is an additional safety net.
...
### Fixed
- **Backspace/Delete key in library thumbnail grid** — Pressing Backspace or Delete while viewing the thumbnail grid in library mode now marks the focused or selected photo(s) for deletion (matching browse mode behaviour). Previously only the fullscreen viewer responded to these keys in library mode.
- **Trash icon missing after viewer deletion** — Photos marked for deletion inside the fullscreen viewer now show the trash icon on their thumbnail immediately after the viewer closes. Previously the `marked-for-deletion` class was not applied to the thumbnail DOM until the next manual re-render.
...
- **Avatar/logo upload now refreshes the site** — Uploading or removing the author portrait or logo in the channel settings for a site-export channel now automatically triggers a site rebuild, so `about.html` and the masthead reflect the change immediately without requiring a manual "Rebuild site" click.
